...
## Project-Specific Learnings (Control-doc)

Based on recent developments in the `Control-doc` project, keep the following context in mind:

- **SSO and Authentication:** The project uses a local Laravel Auth Provider with **Laravel Passport** implementing PKCE clients. This SSO integrates with a **Hono.js** client application. Be mindful of session stability issues (e.g., unexpected logouts after updating user info) and always handle tokens securely.
- **Frontend Validation:** Emphasize real-time (instant) validation for critical form fields (Names, RUT, Passport, Email) to provide immediate visual feedback.
- **Bulk Imports:** The application supports Excel file ingestion using the `xlsx` library. Backend services (e.g., `/workers/bulk`) and frontend hooks (`useWorkerMutations`, `bulkCreate`) work together to map Excel columns to exact TypeScript types like `WorkerInput`.
- **Google Drive Integration:** File access via Google Drive in production depends on long-lived OAuth 2.0 refresh tokens. Any auth issues are usually mitigated by updating `GOOGLE_DRIVE_CLIENT_SECRET` and `GOOGLE_DRIVE_REFRESH_TOKEN` environment variables correctly on Laravel Cloud.
- **Reporting:** Operations typically require detailed visual reports (e.g., "Reporte de Rendición HH") summarizing hours (HH) validated, pending, or rejected per worker. Structure database queries efficiently to aggregate these statistics without N+1 queries.
